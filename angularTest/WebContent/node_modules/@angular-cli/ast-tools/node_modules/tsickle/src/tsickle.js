...
var ClosureRewriter = (function (_super) {
     ...
     */
    ClosureRewriter.prototype.getJSDoc = ;
 ...
 *     bind patterns.
 */
typeToClosure(context, ts.Node, type ?  : ts.Type, destructuring = false);
string;
{
...
var Annotator = (function (_super) {
    ...
    Annotator.prototype.propertyName = ;
    return Annotator;
}(ClosureRewriter));
null;
{
...
visitProperty(namespace, string[], p, ts.Declaration);
{
    var name_5 = this.propertyName(p);
    if (!name_5) {
    ...
    for (var _i = 0, tags_1 = tags; _i < tags_1.length; _i++) {
        var _b = tags_1[_i], tagName = _b.tagName, text = _b.text;
        if (tagName) {
...
visitTypeAlias(node, ts.TypeAliasDeclaration);
{
    if (this.options.untyped)
        return;
...
maybeProcessEnum(node, ts.EnumDeclaration);
boolean;
{
    ...
    var members = new Map();
    var i = 0;
    for (var _c = 0, _d = node.members; _c < _d.length; _c++) {
        var member = _d[_c];
    ...
    this.emit('\n');
    var name_6 = node.name.getText();
    if (node.flags & ts.NodeFlags.Export) {
        this.emit('export ');
    }
    this.emit("type " + name_6 + " = number;\n");
    if (node.flags & ts.NodeFlags.Export) {
        this.emit('export ');
    }
    this.emit("let " + name_6 + ": any = {};\n");
    // Emit foo.BAR = 0; lines.
    for (var _e = 0, _f = util_1.toArray(members.keys()); _e < _f.length; _e++) {
        var member = _f[_e];
        if (!this.options.untyped)
            this.emit("/** @type {number} */\n");
        this.emit(name_6 + "." + member + " = ");
    ...
    for (var _g = 0, _h = util_1.toArray(members.keys()); _g < _h.length; _g++) {
        var member = _h[_g];
        this.emit(name_6 + "[" + name_6 + "." + member + "] = \"" + member + "\";\n");
    }
    return true;
}
            ...
            case ts.SyntaxKind.ModuleDeclaration:
                ...
                switch (decl.name.kind) {
                    case ts.SyntaxKind.Identifier:
                        ...
                        if (name_7 === undefined)
                            break;
                        namespace = namespace.concat(name_7);
